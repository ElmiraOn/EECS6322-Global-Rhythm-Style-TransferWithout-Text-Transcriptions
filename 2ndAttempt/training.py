...
from Model2 import training_2 as Model2
...
def train_model(model1, model2,  train_loader, criterion, optimizer, optimizer2, device, epochs):
        ...
        print("model in training mode 2")
        print("start training...")
        for epoch in range(epochs):
            running_loss = 0.0
            train_iter = iter(train_loader)
            ...
            for i in range(100):
                ...
                optimizer.zero_grad()
                #outputs = model(cep_real, mask_long)
                spect_pred, stop_pred = model2(cep_real.transpose(2,1),
                                                mask_long,
                                                codes_mask,
                                                num_rep_sync,
                                                len_short_sync+1,
            ...
            if (i+1) % 10 == 0:
                    torch.save({'model': model2.state_dict(),
                                'optimizer': optimizer.state_dict()}, f'./Model/{i+1}-A.ckpt')
                    print('Saved model2 into Model ...')  

            print(str(epoch) + "--- Done")
        print("Finished Training stage2...")
        torch.save({'model': model2.state_dict(),
                                'optimizer': optimizer.state_dict()}, f'./Model/Model_Final.ckpt')
        print('Saved model2 into Model ...')  
...
if __name__ == '__main__':
    ...
    train_model(model1, model2, data_loader, criterion, optimizer, optimizer2,  device, num_epochs)