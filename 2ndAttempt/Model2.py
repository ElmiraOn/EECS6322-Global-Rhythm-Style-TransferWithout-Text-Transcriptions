...
def Filter_mean(reps, mask_codes, max_length):
    ...
    index = torch.arange(1, max_length+1, device=reps.device).view(1, 1, -1)
...
class Text_Encoder(nn.Module): #use attentions
    ...
    def forward(self, src, src_lengths, speech_embedd):
        
        speech_embedd = speech_embedd.unsqueeze(0).expand(src.size(0),-1,-1)
        src_spk = torch.cat((src, speech_embedd), dim=-1)
